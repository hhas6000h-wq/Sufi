...
import { Home, PlayCircle, MessageSquare, User as UserIcon, ShieldAlert, Dice5, Radio } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ userRole }) => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[95%] max-w-md z-[100]">
      <nav className="glass rounded-[2.5rem] p-2 flex justify-around items-center shadow-2xl border border-white/10">
        ...
        <NavLink to="/live" className={({ isActive }) => `flex flex-col items-center p-3 rounded-full transition-all duration-300 ${isActive ? 'bg-red-600 text-white scale-110 shadow-lg shadow-red-600/30' : 'text-slate-400 hover:text-white'}`}>
        ...
        {isAdmin && (
          <NavLink to="/admin" className={({ isActive }) => `flex flex-col items-center p-3 rounded-full transition-all duration-300 ${isActive ? 'bg-blue-600 text-white scale-110 shadow-lg shadow-blue-600/30' : 'text-slate-400 hover:text-white'}`}>
