...
const AuthPage: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLoginView) {
      ...
      // --- منطق إنشاء الحساب (Sign Up Logic) ---
      ...
      // إنشاء الكائن الجديد وحفظه في "قاعدة البيانات"
      const newUser = {
        id: 'u_' + Math.random().toString(36).substr(2, 9),
        username: formData.username,
        displayName: formData.displayName || formData.username,
        password: formData.password, // في نظام حقيقي يتم تشفيرها
      ...
      setSuccess('تم إنشاء الحساب بنجاح! جاري التحويل للدخول...');
      setTimeout(() => {
        setIsLoginView(true);
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#020617] relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass rounded-[3.5rem] border border-white/10 p-10 shadow-2xl relative z-10 animate-slide-up">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 text-xs mt-2 font-bold uppercase tracking-widest flex items-center justify-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-5 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl shadow-xl shadow-blue-600/20 active:scale-95 transition-all flex items-center justify-center gap-3 text-sm tracking-wide mt-6"
          >
